...
instruments = ['vc', 'vla', 'vn1', 'vn2']
for i in range(4):
    counts = collections.Counter(voices[i])
    thekeys = counts.keys()
    plt.bar(thekeys, [counts[i] for i in thekeys], color = colors[i], alpha = 0.5, label = instruments[i])
