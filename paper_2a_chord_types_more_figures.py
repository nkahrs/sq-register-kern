...
# extract M/m/7
experiments = [('Major',        list(filter(ismajor_pset, thedata)), 'orange'),
               ('Minor',        list(filter(isminor_pset, thedata)), 'green'),
               ('Dominant 7th', list(filter( isdom7_pset, thedata)), 'magenta')
               ]
...
def run_this_pair(thisdata):
    ...
    # sort each verticality and remove duplicate pitches
    thisdata = [sorted(list(set(i))) for i in thisdata]
    ...
    return [(lowests[i], ranges[i]) for i in range(len(thisdata))] # frequency-space data
...
for thisexpt in experiments:
    ...
    sized_scatter_sub(thisresult, thisexpt[2], thisexpt[0])
...
degcolors = ['red', 'orange', 'yellow', 'green', 'blue', 'magenta', 'brown', 'gray', 'black']
plt.figure(2)
...
cmap = cm.rainbow(np.linspace(0.0, 1.0, len(databykey.keys())+1))
...
for key in databykey.keys():
    ...
    print(key)
    thisresult = run_this_pair(thisdata)
    sized_scatter_sub(thisresult, cmap[whichcolor], key)
    whichcolor += 1
